...
def main():
    ...
    parser.add_argument('--is-blocked', choices=['false','true','none','True','False','None'])
    parser.add_argument('--is-ready', choices=['false','true','none','True','False','None'])
        ...
        for issue in args.issue_id:
            ...
            if args.blocked_by_issues:
                for blocking_issue in args.blocked_by_issues:
                    jira.create_issue_link(inwardIssue=blocker_issue, outwardIssue=tmp.key, type='blocks')

            if args.blocks_issues:
               # outwardIssue == the blocking issue
               for blocker_issue in args.blocks_issues:
                    jira.create_issue_link(inwardIssue=tmp.key, outwardIssue=blocker_issue, type='blocks')
            ...
            if args.is_ready:
                update_fields['customfield_12316542'] = {'value': args.is_ready.capitalize()}


            if args.is_blocked:
                update_fields['customfield_12316543'] = {'value': args.is_blocked.capitalize()}
